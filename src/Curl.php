...
class Curl
{
    ...
    private function goCmdopt()
    {
        ...
        // 请求头
        $method = $this->method ?: ($this->option['method'] ?? ($this->option['source'][CURLOPT_CUSTOMREQUEST] ?? 'GET'));
        $command .= " -X {$method}";
        ...
        } elseif (!empty($this->option['source'][CURLOPT_POSTFIELDS])) {
            $command .= " -d '" . $this->option['source'][CURLOPT_POSTFIELDS] . "'";
        }
        ...
        } elseif (!empty($this->option['source'][CURLOPT_HTTPHEADER])) {
            foreach ($this->option['source'][CURLOPT_HTTPHEADER] as $v) {
                $command .= ' -H "' . $v . '"';
            }
        }
        ...
        } elseif (!empty($this->option['source'][CURLOPT_COOKIE])) {
            $command .= ' -b "' . $this->option['source'][CURLOPT_COOKIE] . '"';
        }
        ...
        } elseif (!empty($this->option['source'][CURLOPT_PROXY])) {
            $command .= ' -x "' . $this->option['source'][CURLOPT_PROXY] . '"';
        }
        ...
        $data = ['response'=>['code'=>-1, 'data'=>$res[1]]];
        ...
        foreach (explode("\r\n", $res[0]) as $v) {
            if (strpos($v, ': ')) {
                ...
                $data['response']['code'] = explode(' ', $v, 2)[1];
            }
        }
     ...
     * 返回[
     *         'async' => 1, //异步返回
     *         'response' => ['code'=>'状态码', 'data'=>'内容', 'location'=>'重定向url'],
    ...
    public function go()
    {
        ...
        $data = ['response'=>['code'=>$httpStatusCode, 'data'=>$res]];
        ...
        if (!empty($this->option['followlocation'])) {
            ...
            $data['response']['location'] = $redirectUrl;
        }
